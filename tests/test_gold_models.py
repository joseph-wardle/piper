...
class TestGoldRenderHealthDaily:
    ...
    def test_required_columns_not_null(self, conn):
        assert _count(conn, "gold_render_health_daily",
                      "event_date IS NULL OR total_jobs IS NULL "
                      "OR success_rate_pct IS NULL OR total_frames IS NULL") == 0
...
class TestGoldFarmPressureDaily:
    ...
    def test_required_columns_not_null(self, conn):
        assert _count(conn, "gold_farm_pressure_daily",
                      "event_date IS NULL OR snapshot_count IS NULL "
                      "OR avg_active_jobs IS NULL OR peak_active_blades IS NULL") == 0

    def test_peak_ge_avg(self, conn):
        assert _count(conn, "gold_farm_pressure_daily",
                      "peak_active_jobs < avg_active_jobs") == 0
        assert _count(conn, "gold_farm_pressure_daily",
                      "peak_active_blades < avg_active_blades") == 0
...
class TestGoldToolReliabilityDaily:
    ...
    def test_required_columns_not_null(self, conn):
        assert _count(conn, "gold_tool_reliability_daily",
                      "event_date IS NULL OR total_invocations IS NULL "
                      "OR success_rate_pct IS NULL") == 0
    ...
    def test_counts_add_up(self, conn):
        assert _count(conn, "gold_tool_reliability_daily",
                      "success_count + error_count != total_invocations") == 0
...
class TestGoldStorageGrowthWeekly:
    ...
    def test_required_columns_not_null(self, conn):
        assert _count(conn, "gold_storage_growth_weekly",
                      "week_start IS NULL OR bucket IS NULL "
                      "OR latest_size_bytes IS NULL OR scan_count IS NULL") == 0

    def test_latest_size_bytes_positive(self, conn):
        assert _count(conn, "gold_storage_growth_weekly",
                      "latest_size_bytes <= 0") == 0
...
class TestGoldDataQualityDaily:
    ...
    def test_required_columns_not_null(self, conn):
        assert _count(conn, "gold_data_quality_daily",
                      "event_date IS NULL OR total_events IS NULL "
                      "OR error_rate_pct IS NULL OR active_users IS NULL") == 0

    def test_error_rate_in_valid_range(self, conn):
        assert _count(conn, "gold_data_quality_daily",
                      "error_rate_pct < 0 OR error_rate_pct > 100") == 0
