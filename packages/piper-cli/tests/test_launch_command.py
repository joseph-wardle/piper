...
def test_a_houdini_this_machine_does_not_have_is_reported_and_nothing_launches(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    def execve(path: Path, _argv: list[str], _environment: dict[str, str]) -> NoReturn:
        raise AssertionError(f"piper became {path}")
    ...
    assert cli.main(("launch", "houdini")) == 1

    assert capsys.readouterr().err == (
        f"piper: general needs Houdini {launch.HOUDINI_VERSION}, which is not installed at "
        f"{tmp_path}; point HFS at a Houdini {launch.HOUDINI_VERSION} instead\n"
    )
