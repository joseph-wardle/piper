...
from piper_studio.storage import pipe_name_refusal
...
def create_asset(
    ...
    *,
    ...
    if existing is not None and not usable_pipe_name(pipe_name):
        raise PiperError(pipe_name_refusal(existing))
    if not usable_pipe_name(pipe_name):
        raise PiperError(
            ...
            + (f", such as {lettered!r}" if (lettered := _lettered(name)) else "")
        )
...
def _lettered(name: str) -> str:
    ...
    return " ".join(lettered + numbered) if lettered else ""
